...
- **⚠️ Restricción Spark:** el proyecto está en plan **Spark (billing OFF)** → NO se pueden desplegar Cloud Functions (Secret Manager 403 por el secret de `sendPinResetOtp`). Por eso el trigger `mirrorPublicProfile` y el onCall `backfillPublicProfiles` quedaron escritos en `functions/src/index.ts` pero SIN desplegar. En Spark, el backfill se corre con el script Admin SDK (no el onCall) y la sincronización continua la cubre el cliente (`updateUserDisplayName` + `syncPublicProfile` en login). Si algún día se activa Blaze, desplegar el trigger cierra el caso del todo.
