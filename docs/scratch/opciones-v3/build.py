#!/usr/bin/env python3
"""
v3 — Opción 5 (MOSAICO) refinada SIN el anillo + los 13 tipos de movimiento.

Decisiones que vienen del usuario:
  · se elige la opción 5 (ficha héroe + teselas + ficha de compartido)
  · FUERA el anillo de progreso → 3 refinamientos del héroe para elegir
  · se conservan: sello rotado y ficha de compartido (avatares + pill)

Los 13 casos salen de las banderas REALES de types/transaction.ts:
  type expense|income · isFixed · isInstallment · isShared + sharedType
  (expense_share | income_claim) · isSentIncome · sentIncomeTransactionId
  · dueño vs invitado · mes pasado bloqueado.
"""
import pathlib
...
.dark{
  ...
  --tP:#EEF6F8; --tS:#9EABAF; --tT:#93A6AB; --noteInk:#9EABAF; --border:#243438; --hair:#223236;
  --track:#123037; --ctaInk:#04191C; --dangerInk:#04191C; --lockBg:#0c272b;
}
.light{
  ...
  --tP:#1A2428; --tS:#6B7280; --tT:#6B7280; --noteInk:#5F6A6E; --border:#DDE8EA; --hair:#E6EEF0;
  --track:#D6EEF2; --ctaInk:#04191C; --dangerInk:#FFFFFF; --lockBg:#EAF7F9;
}
...
.block{margin:48px 0 0;border-top:1px solid #1d2729;padding-top:22px}
.case{margin-top:34px}
.caseh{display:flex;align-items:baseline;gap:10px;flex-wrap:wrap}
.flags{font-family:'DM Mono',monospace;font-size:11px;color:#00BCD4;background:#0f1a1c;
  border:1px solid #1e2a2d;padding:3px 8px;border-radius:6px}
.note-txt{font-size:12px;color:#7d8c90;line-height:1.6;margin-top:6px;max-width:820px}
...
.btn-icon{width:52px;height:52px;flex:0 0 52px;border-radius:26px;color:var(--err);display:grid;place-items:center;
  font-size:19px;background:color-mix(in srgb,var(--err) 14%,transparent)}
...
.amount{font-weight:800;letter-spacing:-2.2px;line-height:1;color:var(--acc);font-size:40px;margin-top:12px}
.amount.sm{font-size:34px;letter-spacing:-1.6px}
...
.figs .fv{font-size:13.5px;font-weight:700;margin-top:4px;font-family:'DM Mono',monospace}
...
.ticks{display:flex;gap:3px;margin-top:14px}
.ticks span{flex:1;height:6px;border-radius:2px;background:var(--track)}
.ticks span.on{background:var(--primary)}
...
.tile .big{font-size:21px;font-weight:800;letter-spacing:-.8px;margin-top:5px;line-height:1}
...
.radio{width:20px;height:20px;border-radius:10px;border:2px solid var(--tT);flex:0 0 20px;display:grid;place-items:center;margin-top:1px}
...
def stamp(txt, ink='var(--expInk)'):
    return (f'<div class="stampwrap"><div class="stamp" style="--stampInk:{ink}">'
            f'{txt}</div></div>') if txt else ''

def hero(acc, kicker, amount, subline, figs=None, prog=None, ticks=None, stamphtml='', small=False):
    """Ficha héroe SIN anillo. `prog` = % (barra fina al pie) · `ticks` = (hechos, total)."""
    novis = '' if (figs or ticks) else ' novis'
    out = [f'<div class="herowrap">{stamphtml}<div class="hero{novis}" style="--acc:{acc}">',
           f'<div class="kick"><i></i>{kicker}</div>',
           f'<div class="amount{" sm" if small else ""} mono">{amount}</div>',
           f'<div class="subline">{subline}</div>']
    if ticks:
        done, total = ticks
        out.append('<div class="ticks">' + ''.join(
            f'<span class="{"on" if i < done else ""}"></span>' for i in range(total)) + '</div>')
    if figs:
...
def t_cat(icon='🍽️', name='Comida', pct='18% del mes'):
    return f'<div class="tile"><div class="lbl">Categoría</div><div class="ic">{icon}</div><div class="s">{name} · {pct}</div></div>'
...
def t_plain(lbl, val, s=''):
    return (f'<div class="tile"><div class="lbl">{lbl}</div><div class="ic" style="font-size:14px;font-weight:700;'
            f'margin-top:6px">{val}</div>' + (f'<div class="s">{s}</div>' if s else '') + '</div>')

def w_shared(lbl, names, pill, avs=(('A', 'var(--primary)'), ('L', 'var(--secondary)'), ('D', 'var(--tertiary)'))):
...
def w_context(txt):
    return (f'<div class="wide"><div class="lbl">Contexto</div>'
            f'<div style="font-size:12.5px;line-height:1.55;color:var(--tS);margin-top:7px">{txt}</div></div>')
...
def cta(kind):
    ...
    if kind == 'fijo':
        return ('<div class="ctarow"><div class="btn btn-primary">✎ Editar</div>'
                ...
                '<div class="btn-icon">⊘</div></div>')
    if kind == 'pedir':
        return ('<div class="ctarow"><div class="btn btn-secondary" style="flex:1">'
                '✉ Pedir que lo borren</div></div>')
    if kind == 'locked':
        return ('<div class="lockbox">🔒<span>Este movimiento ya ocurrió y no puede modificarse.</span></div>')
    return ''

def frame(mode, body_html, cta_html):
    return (f'<div class="frame {mode}">'
            ...
            f'<div class="ctabar">{cta_html}</div></div>')
...
# ═══════════ BLOQUE 1 · tres refinamientos del héroe (sin anillo) ════════════

BASE_SUB = 'tu parte de <b>$248.900</b> · entre 3 personas'
FIGS_CUOTAS = [('Pagado', '$248.901'), ('Resta', '$746.700'), ('Próxima', '22 ago')]

def v_a():  # ticks segmentados + pie de cifras
    return (hero(EXP, 'Gasto compartido', '−$82.967', BASE_SUB, figs=FIGS_CUOTAS,
                 ticks=(3, 12), stamphtml=stamp('CUOTA<br>3 / 12'))
            + '<div class="desc">Mercado del mes</div>'
            + tiles([t_fecha(), t_cat(), t_card()])
            + w_shared('Compartido con', 'Tú, Laura M. y Diego R.', 'A partes iguales · $82.967 c/u')
            + NOTE)

def v_b():  # barra fina al borde inferior + pie de cifras
    return (hero(EXP, 'Gasto compartido', '−$82.967', BASE_SUB, figs=FIGS_CUOTAS,
...
def v_c():  # sin progreso visual: el sello y las cifras lo dicen todo
    return (hero(EXP, 'Gasto compartido', '−$82.967',
                 BASE_SUB + '<br>Cuota <b>3 de 12</b> · próxima el <b>22 de agosto</b>',
                 figs=FIGS_CUOTAS, stamphtml=stamp('CUOTA<br>3 / 12'))
            + '<div class="desc">Mercado del mes</div>'
            + tiles([t_fecha(), t_cat(), t_card()])
            + w_shared('Compartido con', 'Tú, Laura M. y Diego R.', 'A partes iguales · $82.967 c/u')
            + NOTE)
...
VARIANTS = [
    ('5A', 'Ticks segmentados', v_a,
     'Doce marcas: se ve de un golpe cuántas cuotas van y cuántas faltan, sin volver a leer. '
     'Es lo más informativo y lo que más pesa visualmente.'),
    ('5B', 'Barra fina al borde (recomendada)', v_b,
     'El progreso baja a una línea de 3px pegada al borde inferior de la ficha: se percibe sin competir '
     'con la cifra ni con el sello. La ficha se lee como un objeto acabado, no como un widget.'),
    ('5C', 'Sin progreso visual', v_c,
     'Cero gráfico: el sello dice la cuota y una línea de texto dice cuándo cae la próxima. La más '
     'silenciosa y la que mejor aguanta cualquier idioma; pierde el "de un vistazo".'),
]

# ═══════════════════ BLOQUE 2 · los 13 tipos de movimiento ═══════════════════

CASES = []

def case(slug, title, flags, why, body, cta_kind):
    CASES.append(dict(slug=slug, title=title, flags=flags, why=why, body=body, cta=cta_kind))

# 1 · gasto simple con tarjeta y nota
case('gasto-simple', 'Gasto simple', "type:'expense' · cardId",
     'El caso más común. Sin sello, sin ficha de compartido, sin pie de cifras: la ficha héroe queda '
     'limpia y el peso se lo llevan la cifra y la descripción.',
     hero(EXP, 'Gasto · Comida', '−$54.300', 'martes, 22 de julio de 2026 · 19:42')
     + '<div class="desc">Almuerzo con el equipo</div>'
     + tiles([t_fecha(), t_cat(pct='11% del mes'), t_card()])
     + w_context('Es tu <b style="color:var(--tP)">4.º gasto de Comida</b> en julio. '
                 'Llevas <b style="color:var(--tP)">$212.400</b> en esta categoría, un '
                 '<b style="color:var(--tP)">18%</b> de tu mes.')
     + NOTE, 'normal')

# 2 · gasto mínimo: sin tarjeta, sin nota
case('gasto-minimo', 'Gasto mínimo (sin tarjeta, sin nota)', "type:'expense'",
     'El peor caso de escasez de datos: sin cuotas no hay sello ni pie de cifras, y las teselas bajan a dos '
     'repartiéndose el ancho. El hueco que quedaba abajo lo llena la ficha de <b>contexto</b>, que no es relleno: '
     'sale de las transacciones del mes que la vista ya tiene cargadas.',
     ...
     + w_context('Es tu <b style="color:var(--tP)">2.º gasto de Transporte</b> en julio y el '
                 '<b style="color:var(--tP)">más barato</b> de los dos. Llevas '
                 '<b style="color:var(--tP)">$38.500</b> en esta categoría.'),
     'normal')

# 3 · a cuotas, no compartido
case('cuotas', 'Compra a cuotas', "isInstallment · installmentNumber/Total",
     'Aquí sí aparecen el sello y el pie de tres cifras: cuánto llevas pagado, cuánto resta y cuándo cae '
     'la próxima — datos que hoy la pantalla no muestra.',
...
# 4 · gasto fijo mensual
case('fijo', 'Gasto fijo mensual', 'isFixed (· isVirtualFixed en otros meses)',
     'El sello cambia de contenido, no de forma. El pie cuenta la vida del fijo: desde cuándo existe, '
     'cuántos meses lleva y cuándo vuelve a caer. La acción destructiva pasa a «cancelar el fijo» (⊘).',
     None, 'fijo')
CASES[-1]['body'] = (
...
# 5 · fijo de un mes ya cerrado
case('fijo-bloqueado', 'Gasto fijo de un mes cerrado', 'isFixed · isPastMonth',
     'Mismo layout, pero la botonera entera se sustituye por el aviso de bloqueo. El sello se vuelve '
     'neutro para que no parezca una alerta.',
     hero(NEU, 'Gasto fijo · Hogar', '−$120.000', 'sábado, 1 de marzo de 2026 · mes cerrado',
...
# 6 · compartido, yo soy el dueño
case('compartido-dueno', 'Gasto compartido — eres el dueño', "isShared · sharedType:'expense_share' · owner",
     'La cifra héroe es TU parte y la línea de apoyo dice de cuánto sale. La ficha de compartido (la que '
     'te gustó) explica el reparto sin que haya que abrir nada.',
...
case('compartido-invitado', 'Gasto compartido — te lo compartieron', 'isShared · NO owner',
     'No puedes editar ni duplicar lo que no es tuyo: la botonera se reduce a una sola acción, pedirle al '
     'dueño que lo borre. La ficha dice quién lo creó.',
     None, 'pedir')
CASES[-1]['body'] = (
    ...
    + w_shared('Compartido por', 'Laura M. · contigo y Diego R.', 'A partes iguales · $82.967 c/u',
               avs=(('L', 'var(--secondary)'), ('A', 'var(--primary)'), ('D', 'var(--tertiary)'))))

# 8 · compartido + cuotas (el denso)
case('compartido-cuotas', 'Compartido Y a cuotas (el caso más denso)', 'isShared · isInstallment',
     'Los dos estados a la vez: el sello se queda con la cuota, el pie con las tres cifras y la ficha de '
     'compartido con el reparto. Es el techo de densidad de la pantalla y entra sin scroll.',
     v_b(), 'normal')

# 9 · cobro compartido: me deben
case('cobro-dueno', 'Cobro compartido — te deben', "sharedType:'income_claim' · owner",
     'No es un gasto: es plata que va a entrar. Acento de ingreso y sello «TE DEBEN»; la ficha de personas '
     'pasa a decir quién debe cuánto.',
     None, 'normal')
CASES[-1]['body'] = (
    ...
    + tiles([t_fecha(day='18', rest='jul 2026 · 21:30'), t_cat('🎉', 'Ocio', 'no cuenta como gasto')])
    + w_shared('Te deben', 'Laura M. y Diego R.', '$60.000 cada uno · sin pagar',
               avs=(('L', 'var(--secondary)'), ('D', 'var(--tertiary)'))))

# 10 · cobro compartido: le debo
case('cobro-deudor', 'Cobro compartido — le debes', "sharedType:'income_claim' · NO owner",
     'El espejo del anterior y el que más se equivocaba antes: para ti es un egreso, aunque el dueño lo '
     'creó como cobro. Acento de gasto, sello «LE DEBES» y una sola acción.',
     None, 'pedir')
CASES[-1]['body'] = (
    ...
    + w_shared('Le debes a', 'Laura M. · contigo y Diego R.', '$60.000 · pendiente de pago',
               avs=(('L', 'var(--secondary)'), ('A', 'var(--primary)'), ('D', 'var(--tertiary)'))))

# 11 · ingreso normal
case('ingreso', 'Ingreso', "type:'income'",
     'La misma ficha con acento de ingreso y signo +. Sin sello: un ingreso normal no tiene estado que '
     'contar. Las teselas pierden la de tarjeta y ganan aire.',
     hero(INC, 'Ingreso · Salario', '+$4.200.000', 'miércoles, 30 de julio de 2026')
     + '<div class="desc">Salario de julio</div>'
     + tiles([t_fecha(day='30', rest='jul 2026'), t_cat('💰', 'Salario', '87% de ingresos')])
     + w_context('Con este ingreso tu balance de julio queda en '
                 '<b style="color:var(--tP)">+$1.006.000</b> y tu tasa de ahorro en '
                 '<b style="color:var(--tP)">24%</b>.'),
     'normal')

# 12 · ingreso recibido de un amigo
case('ingreso-recibido', 'Ingreso que te envió un amigo', 'isSentIncome · sentByName',
     'Lo creó otra persona, así que tú no lo editas: una sola acción. El sello dice «RECIBIDO» y la ficha '
     'de persona sustituye a la de compartido.',
     None, 'pedir')
CASES[-1]['body'] = (
    ...
    + w_shared('Enviado por', 'Diego R.', 'Llegó a tu balance de julio', avs=(('D', 'var(--tertiary)'),)))

# 13 · envío de dinero a un amigo (gasto del remitente)
case('envio', 'Dinero que le enviaste a un amigo', 'sentIncomeTransactionId · sentIncomeToName',
     'Para ti es un gasto y para el otro un ingreso. El sello dice «ENVIADO» y la ficha de persona dice a '
     'quién; borrarlo aquí también le avisa a la otra persona.',
     None, 'normal')
CASES[-1]['body'] = (
    ...
    + w_shared('Enviado a', 'Diego R.', 'Entró en su balance de julio', avs=(('D', 'var(--tertiary)'),)))

# ═════════════════ BLOQUE 3 · estados de la botonera (borrado) ═══════════════
...
<title>Spendia · Detalle — opción 5 refinada y los 13 tipos</title>
...
nav = ('<div class="topnav"><span class="lg">V3</span>'
       '<a href="#heroe">Héroe sin anillo · 3 refinamientos</a>'
       '<a href="#tipos">Los 13 tipos de movimiento</a>'
       '<a href="#estados">Estados del borrado</a></div>')

doc = [HEAD % CSS, nav,
  '<h1>Spendia · Detalle de transacción — v3</h1>',
  '<p class="sub">Sobre la <b>opción 5</b> que elegiste: <b>fuera el anillo</b>. Primero tres formas de '
  'resolver el progreso de cuotas sin él, y después <b>los 13 tipos de movimiento</b> que maneja la app, '
  'para ver qué pasa cuando el gasto no tiene cuotas, ni tarjeta, ni gente. Todo en dark y light, 390×844.</p>',
  '<div class="keep"><b>Se conservan las piezas que aprobaste:</b> el sello rotado (cambia el texto, nunca la '
  'forma) y la ficha de compartido con la pila de avatares y el pill. <b>Fuera:</b> el anillo de progreso. '
  'La tinta ya viene corregida para modo claro: monto <b>#C62828</b>, pill <b>#006978</b>, '
  'etiquetas <b>#6B7280</b>.</div>']

# bloque 1
doc.append('<section id="heroe"><div class="block"><div class="thesis">Bloque 1 · el héroe sin el anillo</div>'
           '<h2>Tres refinamientos de la ficha héroe</h2>'
           '<p class="meta">Mismo contenido en las tres (cifra, sello, pie de tres cifras); cambia solo cómo '
           'se muestra el avance de las cuotas. Elige una y esa es la que va al código.</p></div>')
for code, name, fn, why in VARIANTS:
    doc.append(f'<div class="case"><div class="caseh"><h3>{code} · {name}</h3></div>'
               f'<p class="note-txt">{why}</p>{pair(fn(), cta("normal"), f"{code} · ")}</div>')
doc.append('</section>')

# bloque 2
doc.append('<section id="tipos"><div class="block"><div class="thesis">Bloque 2 · cobertura completa</div>'
           '<h2>Los 13 tipos de movimiento</h2>'
           '<p class="meta">Cada caso con sus banderas reales del modelo. La regla de composición es siempre '
           'la misma: <b>una ficha por hecho que existe</b>. Si no hay cuotas no hay sello ni pie de cifras; '
           'si no hay tarjeta la fila de teselas se reparte el ancho; si no hay gente no hay ficha ancha. '
           'Se usa el refinamiento <b>5B</b> como base.</p></div>')
...
# bloque 3
doc.append('<section id="estados"><div class="block"><div class="thesis">Bloque 3 · lo que hoy está apretado</div>'
           '<h2>Estados del borrado</h2>'
           '<p class="meta">Hoy el selector de alcance y la confirmación viven dentro de la barra inferior con '
           'texto de 12px y botones de 8px de alto — por debajo del objetivo táctil de 44px. Aquí ocupan el '
           'ancho completo, con radios de 20px y botones de 52px.</p></div>')
...
doc.append("""<script>
/* ?only=slug aísla una sección; ?case=N aísla un caso (solo para revisar) */
addEventListener('DOMContentLoaded',function(){
  var q=new URLSearchParams(location.search), only=q.get('only'), c=q.get('case');
  if(only){document.querySelectorAll('section').forEach(function(s){if(s.id!==only)s.remove()});}
  if(c){var cs=document.querySelectorAll('.case');cs.forEach(function(e,i){if(i!=c-1)e.remove()});}
  if(only||c){document.querySelectorAll('h1,.sub,.keep,.topnav,.block').forEach(function(e){e.remove()});
    document.body.style.padding='20px';}
});
</script>
</body></html>""")

(OUT / 'opciones-v3.html').write_text('\n'.join(doc))
print('escrito opciones-v3.html ·', len(CASES), 'tipos ·', len(VARIANTS), 'refinamientos del héroe')
