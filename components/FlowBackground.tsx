 ...
 * diagonal, muy difuminadas, como tela fluida. A diferencia de Ondas (bandas
...
export default function FlowBackground({ intensity = 'default', speed = 1 }: Props) {
  ...
  const sheets = useMemo(() => ([
    { colorA: colors.primary,   colorB: colors.tertiary, rotate: -16, top: '-14%', height: '70%', base: 0.16, dur: 26000, dir: 1 },
    { colorA: colors.secondary, colorB: colors.info,     rotate: 11,  top: '28%',  height: '64%', base: 0.13, dur: 34000, dir: -1 },
    { colorA: colors.success,   colorB: colors.primary,  rotate: -7,  top: '58%',  height: '60%', base: 0.11, dur: 42000, dir: 1 },
...
function Sheet({ sheet, width, opacityMul, ampMul, speed }: {
  sheet: { colorA: string; colorB: string; rotate: number; top: string; height: string; base: number; dur: number; dir: number };
  ...
  useEffect(() => {
    ...
    const loop = Animated.loop(
      Animated.timing(v, { toValue: 1, duration: sheet.dur * speed, easing: Easing.inOut(Easing.sin), useNativeDriver: false }),
    );
    loop.start();
    return () => loop.stop();
  ...
  return (
    <Animated.View
      style={[
        styles.sheet,
        Platform.OS === 'web' ? ({ filter: 'blur(34px)' } as any) : {},
