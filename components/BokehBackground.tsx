...
export default function BokehBackground({ intensity = 'default' }: Props) {
  ...
  return (
    ...
    >
      {orbs.map((o, i) => (
        <OrbView key={i} orb={o} dark={isDark} travel={travel} at={frozenPhaseFor(i, orbs.length)} />
      ))}
    </View>
  );
}

function OrbView({ orb, dark, travel, at }: { orb: Orb; dark: boolean; travel: number; at: number }) {
  // Cada orbe se queda a una altura distinta del recorrido — congelarlos todos
  // en la misma fase los pondría en fila.
